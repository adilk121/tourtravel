...
$your_date = strtotime($date_of_journey);
$datediff = $now - $your_date;

$com_date=round($datediff / (60 * 60 * 24));

if($com_date==1)
{

    if(date('h:i:s')>="09:00:00" && $pickup_points=="Karol Bagh Metro Station - 09:00 am")
    {?>
       <script>
       alert("You can not select this time period !");
...
$toEmail = $enquiry_email;
$subject = "Booking details from $hostName";
		        $from="info@indiatourismincredible.com";
...
ord_web_url='$website_url',
ord_status='Not Paid',
ord_date=now()
";

$res=db_query($sql);
$newId = "SELECT ord_id FROM tbl_order WHERE ord_status='Not Paid' Order BY ord_id DESC";
$newDbQuery = db_query($newId);
$newsql = mysqli_fetch_array($newDbQuery);
$ordID = $newsql['ord_id'];
//$ordID = mysqli_insert_id();
session_start();
...
<form action="PayUMoney_form.php" method="post" name="SStdFrm">
  ...
  <input type="hidden" name="txnid" value="<?php echo $ordID; ?>" />
  ...
  <input type="hidden" name="surl" value="https://www.indiatourismincredible.com/success.php" size="64" />
  <input type="hidden" name="furl" value="https://www.indiatourismincredible.com/failure.php" size="64" />
