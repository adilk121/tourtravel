...
//if(is_post_back()) {
  if(isset($_REQUEST['submit_model']))  
  {
    $vehicle_model_number=$_REQUEST['vehicle_model_number'];
 ...
 $sql = "update tbl_vehicle_filter set
    ...
    vehicle_status='$vehicle_status'
      where filter_id='$EditID' and  parent_id='$brand_id' ";
 ...
 // $ms="Brand added successfully.";  
  $ms=1;
          ...
          db_query($sql);
header("Location: vehicle_model.php?brand_id=$brand_id");
...
if($EditID!='') {
	$result = db_query("select * from tbl_vehicle_filter where filter_id='$EditID' and parent_id='$brand_id' ");
         ...
         <div class="content-wrapper">
            ...
            <section class="content-header">
               ...
               <div class="header-title">
                  <h1>Add/Edit Model >> <span style="color:green"><?=db_scalar("select vehicle_brand_name from tbl_vehicle_filter where filter_id='$brand_id' ");?></span></h1>
                  <small>Add/Edit model content
...
<input type="text" class="form-control" placeholder="Enter Name" name="vehicle_model_number" id="vehicle_model_number"  value="<?=$vehicle_model_number?>">
                              </div>
...
<select name="vehicle_status" class="form-control" >
 ...
 <div class="col-lg-12 reset-button">
                                 ...
                                 <button type="submit" name="submit_model" id="submit_model" class="btn btn-add">Submit</button>
...
$res=db_query($sql);	
if($res>0){
$_SESSION["msg"]="Selected model is deactivated successfully.";	
}	
}else{
$sql="UPDATE tbl_vehicle_filter SET vehicle_status='Active' WHERE filter_id='$catID' ";	
$res=db_query($sql);	
if($res>0){
$_SESSION["msg"]="Selected model is activated successfully.";	
}	
   ...
   for($i=0; $i<$Arr_size; $i++)
    {
      ...
      $_SESSION["msg"]="selected models are deleted. ";
    }
...
$sql = "select * from  tbl_vehicle_filter  where 1 and parent_id='$brand_id' ";
$sql = apply_filter($sql, $testimonial_name, 'like','vehicle_model_number');
                  ...
                  <div class="col-sm-12">
                     <div class="panel panel-bd lobidrag" data-edit-title='false' data-close='false' data-reload='false'>
                        ...
                        <div class="panel-heading">
                           <div class="btn-group" id="buttonexport">
                              <a href="#">
                                 <h4>Model List >> <span style="color:green"><?=db_scalar("select vehicle_brand_name from tbl_vehicle_filter where filter_id='$brand_id' ");?></span></h4>
                              </a>
                           </div>
                        </div>
...
<p><?=$line_raw["vehicle_model_number"]?></p>
</td>
...
<a href="vehicle_model.php?st=1&pid=<?=$line_raw["filter_id"]?>&brand_id=<?=$brand_id?>"><span class="label-custom label label-default">Active</span></a>
<?php }else{?>
<a href="vehicle_model.php?st=0&pid=<?=$line_raw["filter_id"]?>&brand_id=<?=$brand_id?>"><span class="label-danger label label-default">Inactive</span></a>
<?php }?>
...
<td class="text-center v-middle">
<a href="vehicle_model.php?edit_id=<?=$line_raw["filter_id"]?>&brand_id=<?=$brand_id?>"><button type="button" class="btn btn-add btn-sm" ><i class="fa fa-pencil"></i></button>
</a>                                          
</td>
