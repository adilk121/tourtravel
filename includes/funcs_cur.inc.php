...
function FindStateID($stateName){
	...
	$sres=mysqli_fetch_array(db_query("select state_id from pof_states where state_name='$stateName1'"));
...
function FindStateIDByCode($statecode){
	...
	$sres=mysqli_fetch_array(db_query("select state_id from pof_states where state_code='$statecode1'"));
...
function FindStateName($stateID){
	...
	$sres=mysqli_fetch_array(db_query("select state_name from pof_states where state_id='$stateID'"));
...
function FindStateCode($stateID){
	...
	$sres=mysqli_fetch_array(db_query("select state_code from pof_states where state_id='$stateID'"));
...
function FindCountryName($contID){
	...
	$sres=mysqli_fetch_array(db_query("select countries_name from ques_countries where countries_id='$contID'"));
...
/*searched person detail*/
	...
	$searched_person_detail = mysqli_fetch_array($searched_person_result);
...
/*searching person detail*/
	...
	$searching_person_detail = mysqli_fetch_array($searching_person_result);

	$searching_person_result2=db_query("SELECT * FROM pof_relation_type WHERE rel_user_id='$_SESSION[sess_user_id]'");

	$searching_person_detail2 = mysqli_fetch_array($searching_person_result2);
...
return mysqli_fetch_array($res_page);
}
...
function parent_search($slno, $table_name, $name_field, $primery_field, $search_field,$link,$html){ //for searching parent.
	$db=mysqli_fetch_array(db_query("select * from $table_name where $primery_field='$slno'"));
	echo mysqli_error();
